...
fn test_timestamp_to_utc() {
    ...
    assert_eq!(result, Utc.with_ymd_and_hms(2023, 8, 2, 14, 19, 23).unwrap());
}

pub(crate) fn format_utc_msec(msec: Timestamp, tz: &chrono_tz::Tz, all_day: bool, name_hint: &str) -> Result<String, Error> {
...
fn from_ical_get_timestamp(prop: &ical::property::Property, prop_hint: &str) -> Result<(Timestamp, bool), Error> {
    ...
    Err(Error::InvalidData(format!("Unknown timestamp format value: {}", prop_hint)))
}
...
fn test_from_ical_get_timestamp() {
    ...
    let prop = ical_property!("DTSTART", "20230829T043000", ical_param!("TZID", "Europe/Berlin"));
...
pub(crate) fn from_ical_get_event_bounds(event: &IcalEvent) -> Result<(Timestamp, Timestamp, bool), Error> {
    let prop = event.get_property("DTSTART")
    ...
    let (end_ts, end_all_day) = if let Some(prop) = event.get_property("DTEND") {
        ...
        return Err(Error::InvalidData("Missing property: DTEND or DURATION".into()));
    };
...
fn test_from_ical_get_event_bounds() {
    ...
    event.properties.push(ical_property!("DTSTART", "20230802T141923Z"));
    ...
    event.properties.push(ical_property!("DTSTART", "20230802T141923Z"));
    event.properties.push(ical_property!("DURATION", "PT2H"));
    event.properties.push(ical_property!("DTEND", "20230802T171923Z"));
    let (start, end, all_day) = from_ical_get_event_bounds(&event).unwrap();
    assert_eq!(start, 1690985963000);
    ...
    let mut event = IcalEvent::new();
    event.properties.push(ical_property!("DTSTART", "20230802T141923Z"));
    event.properties.push(ical_property!("DTEND", "20230802T181923Z"));
    let (start, end, all_day) = from_ical_get_event_bounds(&event).unwrap();
    assert_eq!(start, 1690985963000);
    ...
    let mut event = IcalEvent::new();
    event.properties.push(ical_property!("DTSTART", "20230802", ical_param!("VALUE", "DATE")));
    event.properties.push(ical_property!("DTEND", "20230803", ical_param!("VALUE", "DATE")));
...
pub(crate) fn from_ical_get_exdate(ical_event: &IcalEvent) -> Result<Option<Vec<Timestamp>>, Error> {
...
fn test_from_ical_get_exdate() {
    let mut event = IcalEvent::new();
    event.properties.push(ical_property!("EXDATE", "20230829", ical_param!("VALUE", "DATE")));
    event.properties.push(ical_property!("EXDATE", "20230830", ical_param!("VALUE", "DATE")));
