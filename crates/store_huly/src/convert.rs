...
pub(crate) fn from_ical_get_alarms(event: &IcalEvent) -> Result<Option<Vec<Timestamp>>, Error> {
    ...
    for ical_alarm in &event.alarms {
        let prop = ical_alarm.get_property("TRIGGER")
            .ok_or_else(|| Error::InvalidData("Missing property: TRIGGER".into()))?;
        if let Some(param) = prop.params.as_ref() {
            if param.iter().any(|(name, vals)| if name == "RELATED" {
                vals.iter().any(|val| val != "START")
            } else {
                false
            }) {
                return Err(Error::InvalidData("Only triggers related to the start of the event are supported".into()));
            }
        }
        let value = prop.value.as_ref()
            .ok_or_else(|| Error::InvalidData("Missing value: TRIGGER".into()))?;
        let dist = parse_duration(value)
            .map_err(|_| Error::InvalidData("Only triggers in the form of DURATION are supported".into()))?;
        if dist > chrono::TimeDelta::zero() {
            return Err(Error::InvalidData("Only triggers that are before the events are supported".into()));
        }
...
fn make_ical_event(event: &HulyEventData, tz: &chrono_tz::Tz) -> Result<IcalEvent, Error> {
    ...
    let mut ical_event = ical_event
        ...
        .set(ical_property!("DESCRIPTION", &event.description))
    ...
    if let Some(exdate) = &event.exdate {
        for dt in exdate {
            ...
            ical_event = ical_event.set(ical_property!("EXDATE", dt_str, ical_param!("VALUE", "DATE")));
        }
    }
    ...
    if let Some(reminders) = &event.reminders {
        for reminder in reminders {
            ...
            alarm.properties.push(ical_property!("TRIGGER", format_duration_rfc5545(-reminder), ical_param!("RELATED", "START")));
        ...
        if let Some(instances) = &self.instances {
            for instance in instances.iter() {
                ...
                if instance.is_cancelled.unwrap_or(false) {
                    ...
                    ical_event.add_property(ical_property!("EXDATE", dt_str, ical_param!("VALUE", "DATE")));
                    continue;
                }
                ...
                ical_instance.add_property(ical_property!("RECURRENCE-ID", orig_start, ical_param!("TZID", tz.name())));
